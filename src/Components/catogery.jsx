      ...
      {category.length > 0 &&
        category.map((c) => (
          <Link
            to="/Products"
            key={c.id}
            className="relative w-full lg:w-[30%] cursor-pointer bg-center bg-cover min-h-[50rem] max-h-[70rem] lg:min-h-[25rem] lg:max-h-[70rem] xl:min-h-[35rem] xl:max-h-[60rem] group overflow-hidden"
            style={{ backgroundImage: `url(${c.image})` }}
          >
            <div
              className="absolute inset-0 transition-transform duration-500 group-hover:scale-110 group-hover:opacity-100 bg-cover bg-center"
              style={{ backgroundImage: `url(${c.image})` }}
            ></div>
